---
name: commit
description: Create git commits with structured messages after completing tasks.
model: haiku
color: green
tools: ["Bash"]
---
...
**Delegated invocation** when orchestrator or task agent completes work.
...
8. **Return result**
   - Success: Return commit hash only (e.g., "abc123f")
   - Failure: Return error message with diagnostic info
...
- **Terse output**: Return only commit hash on success, detailed error on failure
...
When invoked by orchestrator with literal commit message:
...
**Execution:**
1. Skip steps 1-3 (analysis already done by orchestrator)
2. Run pre-commit checks (step 4)
3. Stage changes (step 5)
4. Create commit with provided message (step 6)
5. Verify and return result (steps 7-8)

**Output:**
- Success: `<commit-hash>`
- Failure: `Error: <diagnostic info>`

This mode enables weak orchestration where sonnet analyzes changes, drafts message, and delegates execution to haiku.
