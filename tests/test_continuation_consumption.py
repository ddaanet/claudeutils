"""Unit tests for continuation consumption protocol (cooperative skill protocol).

Tests the protocol that skills use to consume continuation entries from additionalContext
or from [CONTINUATION: ...] suffix in Skill args. Based on design Component 3 test scenarios
and FR-2 (sequential execution - peel-first-pass-remainder protocol).
...
def parse_continuation_string(continuation_str: Optional[str]) -> Optional[List[Dict[str, str]]]:
...
def peel_continuation(args_with_continuation: str) -> Tuple[Optional[Dict[str, str]], Optional[str]]:
    """Peel first entry from continuation in args, return (target_entry, remainder_str).

    Input: args potentially containing [CONTINUATION: /a, /b, /c]
    Output: ({'skill': 'a', 'args': ''}, '[CONTINUATION: /b, /c]')

    Returns:
        (None, None) if no continuation
        (target_dict, remainder_str) where remainder_str is formatted [CONTINUATION: ...] or None if empty
    """
...
class TestParseConsumptionFormat:
    ...
    def test_parse_multiple_entries_with_args(self) -> None:
        ...
        entries = parse_continuation_string('[CONTINUATION: /plan-adhoc foo, /orchestrate, /commit]')
    ...
    def test_parse_complex_args(self) -> None:
        ...
        entries = parse_continuation_string('[CONTINUATION: /design plans/myplan --verbose]')
...
class TestPeelFirstEntry:
    """Tests for peeling first entry from continuation (peel-first-pass-remainder protocol)."""
    ...
    def test_peel_first_of_three(self) -> None:
        """Peel first entry from three-entry continuation → (target, [CONTINUATION: /b, /c])."""
    ...
    def test_peel_with_args(self) -> None:
        ...
        target, remainder = peel_continuation('[CONTINUATION: /handoff --commit, /commit]')
    ...
    def test_peel_preserves_complex_args_in_remainder(self) -> None:
        ...
        target, remainder = peel_continuation('[CONTINUATION: /design foo, /plan-adhoc --verbose bar, /commit]')
...
class TestConsumptionProtocol:
    ...
    def test_skill_must_not_include_continuation_in_task_tool(self) -> None:
        ...
        Enforcement would be via PreToolUse hook, which is out of scope for this phase.
        """
        ...
        # WRONG:
        #   Task(task_prompt=f"Do work {remaining_continuation}")
        #
        # RIGHT:
        #   (Do work without continuation in Task prompt)
        #   Then tail-call with continuation:
        #   Skill(skill="next", args=f"args {remaining_continuation}")
...
class TestConsumptionEdgeCases:
    ...
    def test_skill_name_with_underscore(self) -> None:
        ...
        target, remainder = peel_continuation('[CONTINUATION: /plan_adhoc arg, /commit]')
    ...
    def test_args_with_multiple_spaces(self) -> None:
        ...
        entries = parse_continuation_string('[CONTINUATION: /design plans/foo --verbose  --quiet]')
...
/commit]"""
    ...
    def test_peel_with_extra_whitespace_preservation(self) -> None:
        ...
        target, remainder = peel_continuation('[CONTINUATION:   /design   plans/foo   ,   /commit   ]')
...
class TestDesignReferences:
    ...
    def test_fr3_continuation_consumption_cooperative_skill_protocol(self) -> None:
        ...
        if remainder:
            tail_call_args = f"my-args {remainder}"
        else:
            tail_call_args = "my-args"

        # Verify structure for Skill tool invocation
        assert 'next-skill' is not None  # target skill is available
