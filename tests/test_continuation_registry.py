"""Unit tests for continuation registry builder (userpromptsubmit-shortcuts.py).

Tests the registry building logic that scans skills and extracts cooperation metadata.
Based on design Component 2 test scenarios.
"""
...
""")
        ...
        with patch.dict(os.environ, {'CLAUDE_PROJECT_DIR': str(tmp_path)}):
            ...
            cache_path = get_cache_path([str(skills_path / "design" / "SKILL.md"),
...
""")
        ...
        with patch.dict(os.environ, {'CLAUDE_PROJECT_DIR': str(tmp_path)}):
            cache_path = get_cache_path([str(skills_path / "legacy" / "SKILL.md")], str(tmp_path))
            if cache_path.exists():
                cache_path.unlink()
...
""")
        ...
        with patch.dict(os.environ, {'CLAUDE_PROJECT_DIR': str(tmp_path)}):
            cache_path = get_cache_path([str(skills_path / "commit" / "SKILL.md")], str(tmp_path))
            if cache_path.exists():
                cache_path.unlink()
    ...
    def test_build_registry_uses_cache(self, tmp_path: Path, monkeypatch) -> None:
...
""")
        ...
        with patch.dict(os.environ, {'CLAUDE_PROJECT_DIR': str(tmp_path)}):
            cache_path = get_cache_path([str(skills_path / "myskill" / "SKILL.md")], str(tmp_path))
            if cache_path.exists():
                cache_path.unlink()
...
class TestPluginSkillDiscovery:
    ...
    def test_get_plugin_install_path_not_found(self, tmp_path: Path) -> None:
        """Get plugin install path returns None if plugin not in installed list."""
