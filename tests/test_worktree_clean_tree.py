...
def test_merge_dirty_tree_gate_rejects_source_changes(
    repo_with_submodule: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Merge exits 1 when source files are modified.

    Behavioral verification that merge enforces clean working tree as Phase 1
    pre-check. Dirty source files (not session context files) should cause merge
    to reject and exit 1 with error message to stderr.
    """
    monkeypatch.chdir(repo_with_submodule)

    # Create a worktree branch first
    subprocess.run(
        ["git", "branch", "test-slug"],
    ...
    # Modify the source file (dirty tree)
    (src_dir / "init.py").write_text("# modified\n")

    # Run merge command
    runner = CliRunner()
    result = runner.invoke(worktree, ["merge", "test-slug"])

    # Should exit 1 with error about dirty tree
    assert result.exit_code == 1
    assert "dirty" in result.output.lower() or "uncommitted" in result.output.lower()


def test_merge_session_files_exempt_from_dirty_gate(
    repo_with_submodule: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Merge accepts session file modifications in dirty tree gate.

    Behavioral verification that merge Phase 1 pre-check exempts session context
    files (agents/session.md, agents/jobs.md, agents/learnings.md) from dirty
    tree rejection.
    """
    monkeypatch.chdir(repo_with_submodule)
    ...
    # Modify only session files (should be exempt)
    agents_dir = repo_with_submodule / "agents"
    (agents_dir / "session.md").write_text("# Session\nModified\n")
    (agents_dir / "jobs.md").write_text("# Jobs\nModified\n")

    # Run merge command - should pass the clean tree gate
    runner = CliRunner()
    result = runner.invoke(worktree, ["merge", "test-slug"])

    # Should NOT exit 1 due to dirty tree (session files exempt)
    # Will fail for other reasons (branch/worktree checks), but not dirty tree
    assert result.exit_code != 1 or (
        "dirty" not in result.output.lower()
        and "uncommitted" not in result.output.lower()
    )
