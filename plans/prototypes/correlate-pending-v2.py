...
PROJECT_DIRS = [Path.home() / "code" / "claudeutils"]
...
def scrape_pending_directives() -> list[dict]:
    results = []
    uuid_pat = re.compile(r"^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.jsonl$")
...
def get_all_insertions() -> list[dict]:
    ...
    focused_result = subprocess.run(
        ['git', 'log', '--all', '--oneline', '--no-merges', '--grep=Focused session', '--', 'agents/session.md'],
        capture_output=True, text=True
    )
    focused_hashes = {line.split()[0] for line in focused_result.stdout.strip().split('\n') if line}
...
def keyword_match(directive_text: str, task_name: str) -> float:
    ...
    skip = {'update', 'add', 'fix', 'use', 'the', 'for', 'with', 'and', 'that', 'all',
            'after', 'from', 'into', 'when', 'should', 'must', 'not', 'has', 'have',
            'run', 'make', 'get', 'set', 'new', 'also', 'its', 'this', 'been'}
...
def main():
    ...
    for d in directives:
        ...
        if best_score >= 0.15:
            ...
            unmatched.append({**d, "best_candidate": best["task"], "best_score": best_score})
    ...
    for m in sorted(matches, key=lambda x: x["directive"]["date"]):
        ...
        print(f"\n[{d['date']}] p: {d['text'][:70]}")
        print(f"  → [{ins['pos']:12s}] {ins['idx']}/{ins['total']-1} {ins['task'][:55]}{batch} (sim={m['score']:.2f})")
