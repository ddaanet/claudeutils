...
| FR-4: `rm` exit codes (0/1/2) | Phase 1 | Cycles 1.4-1.7 |
| FR-5: CLI never suggests destructive commands | Phase 1 | Cycle 1.8 |
| FR-6: Phase 4 refuses single-parent when merge expected | Phase 1 | Cycles 1.9-1.10 |
| FR-7: Post-merge ancestry validation | Phase 1 | Cycle 1.11 |
| FR-8: `rm` reports removal type in success message | Phase 1 | Cycle 1.7 |
...
**Complexity:** ~11 TDD cycles, 3 implementation files + test files
...
- Cycle 1.4: Guard refuses unmerged real history (exit 1, stderr message)
- Cycle 1.5: Guard allows merged branch removal (exit 0, `git branch -d`)
- Cycle 1.6: Guard allows focused-session-only removal (exit 0, `git branch -D`, type message)
- Cycle 1.7: Success messages differentiate removal types
- Cycle 1.8: No `git branch -D` in stderr output for any case

#### Track 2: Merge Correctness (merge.py)

- Cycle 1.9: MERGE_HEAD checkpoint — refuse single-parent commit for unmerged branch (exit 2)
- Cycle 1.10: Already-merged idempotency — allow single-parent commit when branch merged
- Cycle 1.11: Post-merge ancestry validation — `_validate_merge_result(slug)` verifies ancestor
- Cycle 1.12: Parent repo file preservation — branch with parent + submodule changes → all files present (regression test)

**Checkpoint:** After Cycle 1.12 — Light checkpoint (Fix + Functional)
...
- **Dependencies:** Cycles 1.1-1.8 (Track 1) and 1.9-1.12 (Track 2) are independent until checkpoint
...
**Track independence:** Track 1 (cycles 1.1-1.8) and Track 2 (cycles 1.9-1.12) can be developed in parallel during expansion. Both track test files can be created concurrently. Only the checkpoint requires both tracks complete.

**Cycle consolidation:** Cycles 1.1 (shared helper) is a prerequisite for both tracks but isolated in utils.py. Cycles 1.7-1.8 are refinements that could merge into 1.6 if delta is minimal.
