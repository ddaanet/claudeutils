...
**Goal:** Add `_format_conflict_report(conflicts: list[str], slug: str) -> str` function to `merge.py`. Call it from the conflict exit paths (Phase 3 source conflicts and Phase 1 `parent_conflicts` state). Output provides context the agent needs to resolve conflicts: file list with type, per-file diff stats, branch divergence, hint. Implements FR-4.

**Files:** `src/claudeutils/worktree/merge.py`, `tests/test_worktree_merge_conflicts.py`

**Depends on:** Cycle 3.1 (exit 3 path exists and is tested before output format is added).

**Common context:**
- `_format_conflict_report` is a pure-query function: reads git state, formats report string. No mutation.
- Output fields (FR-4 acceptance criteria):
  1. Conflicted file list with conflict type — from `git status --short` (lines starting with `UU`, `AU`, `UD`, etc.)
  2. Per-file diff stat — `git diff --stat MERGE_HEAD -- <file>` for each conflicted file
  3. Branch divergence — `git rev-list --count HEAD..slug` (commits on branch since merge-base) and `git rev-list --count slug..HEAD` (commits on main since merge-base)
  4. Hint — `f"Resolve conflicts, git add, then re-run: claudeutils _worktree merge {slug}"`
- Call sites: (a) `_phase3_merge_parent` `if conflicts:` block (Phase 3 Cycle 3.1), (b) `parent_conflicts` branch in `merge()` (Phase 1 Cycle 1.3 routing stub)
- All test repos: real git repos, no mocks.
