...
- **D-4:** Cache in project-local `tmp/`, mtime invalidation → Cycles 2.1–2.2
- **D-5:** Section extraction from decision files → Cycle 1.4
- **D-6:** Entry count cap (max 3) → Cycle 1.3
- **D-7:** Dual-channel output (additionalContext + systemMessage) → Cycle 1.5, 3.1
...
- Cycle 1.4: promote `_extract_section` → `extract_section` in `when/resolver.py` (needed for content extraction from decision files; error paths covered by parametrized test)
...
- **Cycle 1.3: Score, rank, and cap candidates via score_relevance** [FR-2, FR-6, D-1, D-6]
  - RED: `test_score_candidates_ranks_by_relevance_and_filters` — parametrized: (1) given 3 candidates with different keyword overlaps, `score_and_rank()` returns list sorted by score descending, entries below threshold 0.3 excluded, assert high-overlap entry first (score ~0.8), low-overlap entry excluded (score 0.1); (2) given 5 candidates all above threshold with max_entries=3, returns only top 3 by score, assert len(result) == 3 and all scores >= score of 4th entry.
  - GREEN: `score_and_rank(prompt_keywords: set[str], candidates: set[IndexEntry], threshold: float, max_entries: int | None = None) -> list[tuple[IndexEntry, RelevanceScore]]`. Call `score_relevance()` per candidate with session_id="hook", filter by threshold, sort descending, slice to max_entries if set.
  - Depends on: 1.2

- **Cycle 1.4: Resolve matched entries with error-path coverage** [FR-3, D-5]
  - RED: `test_resolve_entries` — parametrized: (1) given a matched entry referencing a decision file (created in tmp_path), `resolve_entries()` returns list of resolved content strings containing the section heading and body text, assert content contains heading text and at least one body line; (2) given entry referencing nonexistent file, `resolve_entries()` returns empty list (no errors raised); (3) given entry referencing file with missing section heading, `resolve_entries()` returns empty list (no errors raised).
  - GREEN: `resolve_entries(entries: list[tuple[IndexEntry, RelevanceScore]], project_dir: Path) -> list[ResolvedEntry]`. Promote `_extract_section` → `extract_section` in `when/resolver.py` (update internal callers at lines 117, 225). For each entry: construct heading from entry key (try both `## When {key}` and `## How to {key}` — see heading reconstruction note above), call `extract_section(file_path, heading)`. `extract_section()` already returns empty string on missing file/section — filter out empty results.
  - Depends on: 1.3

- **Cycle 1.5: Format dual-channel output** [FR-7, D-7, C-2]
...
- **Cycle 2.2: Cache hit and invalidation** [FR-4]
  - RED: `test_cache_behavior` — parametrized: (1) call `get_or_build_index()` twice with same inputs, monkeypatch `parse_memory_index` to track call count, assert parse called exactly once (second call uses cache); (2) build cache, modify source file (touch), call `get_or_build_index()` again, assert parse called again (cache invalidated via mtime comparison).
  - GREEN: Load cache file, validate timestamp > source mtime, return cached data on hit. If source newer, invalidate and rebuild.
  - Depends on: 2.1
...
|-------|--------|------------|-------|
| Phase 1 | 5 | Medium | New module, reuses existing infrastructure |
| Phase 2 | 2 | Low | Follows established caching pattern |
| Phase 3 | 3 | Medium | Hook integration, test fixture setup for hook invocation |

**Total: 10 cycles across 3 phases.**

## Expansion Guidance

The following recommendations should be incorporated during full runbook expansion:

**Consolidation applied** (see `reports/simplification-report.md`):
- Cycles 1.4+1.5 merged → parametrized Cycle 1.4 (resolve + error paths)
- Cycles 1.3+1.6 merged → Cycle 1.3 (score/rank/cap)
- Cycles 2.2+2.3 merged → Cycle 2.2 (cache hit + invalidation)
...
- **Cycle 1.5 systemMessage format:** Terminal constraint ~60 chars for content after "UserPromptSubmit says: " prefix. The `system_message` field in `TopicMatchResult` becomes one entry in `system_parts` — the hook joins system_parts with `" | "`. The topic block must be pre-formatted as one string with internal newlines. Refer to D-7 for format spec.

**Checkpoint guidance:**
- Phase 1 light checkpoint (after cycle 1.5): `just dev` + verify `topic_matcher.py` exports `match_topics()` entry point
- Phase 2 light checkpoint (after cycle 2.2): `just dev` + verify cache file creation in `tmp/`
- Phase 3 full checkpoint (after cycle 3.3): `just dev` + run integration tests + verify dual-channel output
