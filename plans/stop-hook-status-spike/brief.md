...
**Mechanism:** Agent outputs trigger string ("Status.") as last line → Stop hook detects via `grep` on `last_assistant_message` → hook runs `claudeutils _status` → returns `systemMessage` with output → user sees it directly.
