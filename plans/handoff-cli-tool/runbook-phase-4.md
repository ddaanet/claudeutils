...
Tests use real git repos via `tmp_path` — committed detection requires `git diff HEAD`.

**Assertions — overwrite mode (no prior diff):**
- First handoff or prior content already committed → `write_completed(session_path, new_lines)` overwrites `## Completed This Session` section entirely with `new_lines`

**Assertions — append mode (old removed by agent):**
- Agent cleared old completed content, new additions present in stdin → `write_completed()` appends new lines to section (does not restore old content)
