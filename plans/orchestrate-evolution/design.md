...
| FR-8 | Yes | Ping-Pong TDD Orchestration (D-5), agent roles and loop |
| FR-8a | Yes | Ping-Pong TDD Orchestration (D-5), RED gate |
| FR-8b | Yes | Ping-Pong TDD Orchestration (D-5), GREEN gate |
...
### Ping-Pong TDD Orchestration (D-5, Phase 2)

**Depends on:** Phase 1 infrastructure (agent caching D-2, remediation D-3, verification script).
...
Per TDD cycle within a phase:

```
1. Dispatch tester with test spec (step-N-test.md)
...
#### Step File Separation

prepare-runbook.py splits TDD cycles into two step files per cycle:
- `step-N-test.md` — test spec, dispatched to tester agent
- `step-N-impl.md` — implementation hints, dispatched to implementer agent

Orchestrator plan marks each with a role:
```
- step-1-1-test.md | Phase 1 | sonnet | TEST
- step-1-1-impl.md | Phase 1 | sonnet | IMPLEMENT
- step-1-2-test.md | Phase 1 | sonnet | TEST
- step-1-2-impl.md | Phase 1 | sonnet | IMPLEMENT | PHASE_BOUNDARY
```

The orchestrator dispatches TEST steps to `<plan>-tester`, IMPLEMENT steps to `<plan>-implementer`. No step content reading by orchestrator — role marker determines agent selection.

#### RED Gate Script

**Location:** `agent-core/skills/orchestrate/scripts/verify-red.sh`

**Contract:**
- Input: test file path (argument)
...
| File | Action | Description |
|------|--------|-------------|
| `agent-core/skills/orchestrate/SKILL.md` | Extend | TDD orchestration loop (alternating dispatch, RED/GREEN gates) |
| `agent-core/bin/prepare-runbook.py` | Extend | TDD agent generation (4 types), step file splitting (test/impl) |
...
**Phase 2:**
- **prepare-runbook.py:** Unit tests for TDD agent generation (4 agent types), step file splitting (test/impl separation)
- **RED gate script:** Unit test — test file that fails → exit 0, test file that passes → exit 1
- **GREEN gate:** No new script — composes `just test` (existing) + `verify-step.sh` (Phase 1). Tested via TDD loop integration test
- **Orchestrate skill TDD loop:** Manual integration test — run against a TDD runbook with at least 2 cycles
- **Agent resume:** Validate context reuse across cycles (manual, observe agent behavior)
...
**Phasing:** Phase 1 (foundation) ships sequential orchestration, agent caching, remediation, verification. Phase 2 (ping-pong TDD) adds alternating agent loop, role-specific agents, RED/GREEN gates. Phase 2 depends on Phase 1 infrastructure.
