...
1. **Missing deletion of `pretooluse-symlink-redirect.sh`**
   ...
   - Problem: Outline line 162 explicitly requires deleting `pretooluse-symlink-redirect.sh` from `agent-core/hooks/` as part of symlink cleanup. Step 6.1 listed 6 items but omitted this deletion, leaving a dead hook script on disk.
   - Fix: Added item 7 with `rm agent-core/hooks/pretooluse-symlink-redirect.sh`, cross-reference to hooks.json entry removal, validation command `test ! -f agent-core/hooks/pretooluse-symlink-redirect.sh && echo OK`, and added to Expected Outcome.
   - **Status**: FIXED

2. **Vacuous `delegation.md` item in Step 6.2**
   - Location: Step 6.2, Prerequisites read-list and item 4
   - Problem: Step 6.2 instructed reading `agent-core/fragments/delegation.md` and "updating examples referencing `sync-to-parent`." Grep confirms `delegation.md` contains zero `sync-to-parent` references — the item is a dead instruction that wastes executor time and risks scope creep.
...
1. **Ambiguous symlink grep validation in Step 6.2**
   ...
   - Problem: `grep -r 'symlink' agent-core/fragments/claude-config-layout.md` with qualifier "or only in historical context" — this is not a binary pass/fail criterion. An executor cannot determine what "historical context" means.
   - Fix: Changed to `grep 'sync-to-parent\|Symlinks in .claude' agent-core/fragments/claude-config-layout.md` targeting the exact subsection heading and content being removed. Zero matches = pass.
   - **Status**: FIXED
...
2. **Vague FR-7 `@`-reference check in Step 6.3**
   ...
   - Fix: Replaced with two concrete grep commands: one to find `@agent-core/` references in CLAUDE.md and rules, one to enumerate `^@` references within fragments/skills — both with verification action stated.
   - **Status**: FIXED
...
- Step 6.1 Validation — Added `test ! -f agent-core/hooks/pretooluse-symlink-redirect.sh && echo OK`
