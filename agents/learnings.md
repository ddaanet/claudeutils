...
- Correct pattern: `remerge_session_md(slug)` now runs in phase 4 on ALL merge paths, structurally merging session.md via `_merge_session_contents`. Manual validation no longer required for the known failure modes (WT section drops, task list loss, blocker bleed). Still validate novel merge scenarios not yet covered by tests.
- Evidence: Merge 1 dropped "Update grounding skill" from Worktree Tasks. Merge 2 left orphaned entry + malformed blocker line. Fix: `remerge_session_md` in resolve.py, called from `_phase4_merge_commit_and_precommit`.
