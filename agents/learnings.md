...
## When batching RED tests in TDD
- Anti-pattern: Writing all test changes in one RED phase, confirming only one failure, then implementing everything. Tests that were GREEN before implementation (characterization tests like `designed_with_outline`) prove nothing — they were never RED.
- Correct pattern: One acceptance criterion per RED→GREEN cycle. Verify each new assertion fails independently before implementing its fix. Characterization tests (confirming existing behavior unchanged) are added separately, acknowledged as already-GREEN.
- Rationale: Batching masks vacuous assertions. A test that passes before implementation could be testing the wrong thing — the RED phase exists to prove the assertion is meaningful.