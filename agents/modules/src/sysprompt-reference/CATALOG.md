...
Reference extraction from Claude Code system prompt (v2.0.75).
These modules preserve all instruction categories for potential inclusion in role prompts.
...
**OUTPUT_STYLE_CONFIG**: When null, default instructions apply (tone, style, doing tasks).
When set, custom output style replaces these sections.
...
Task agent prompt is a **minimal replacement** for the main system prompt.
Rules NOT in Task agent are effectively "interactive-only" in default Claude Code.
